...
// import UsersClientPage from './Resource-typesClientPage';
...
//   const  resource_types= data.map(u => ({
//     id: u.resource_type_id.toString(),
//     type_name: u.type_name
//   }));

//   return <Resource-typesClientPage initialResource-types={resource_types} />;
// }
...
export default async function ResourceTypesPage() {
  const data = await prisma.resource_types.findMany({
    ...
    orderBy: {
      resource_type_id: 'desc',
    },
  });

  // Convert DB → Client format
  const resourceTypes = data.map(rt => ({
    id: rt.resource_type_id.toString(),
    name: rt.type_name,
    description: '', // you don’t have description column
  }));

  return <ResourceTypesClient initialResourceTypes={resourceTypes} />;
}
