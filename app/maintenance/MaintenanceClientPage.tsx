...
export default function MaintenanceClientPage({
  ...
  const handleEdit = (maint: Maintenance) => {
    ...
    setFormData({
      ...
      scheduledDate: toDateInputValue(maint.scheduledDate),
      status: (maint.status || 'Scheduled') as 'Scheduled' | 'In Progress' | 'Completed',
      notes: maint.notes,
    });
    setShowModal(true);
  };
                ...
                {filteredMaintenance.length === 0 ? (
                  ...
                  filteredMaintenance.map((maint) => (
                    <tr key={maint.id}>
                      ...
                      <td>{formatDisplayDate(maint.scheduledDate)}</td>
